...
export class PanierComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.panierService.getProduitPanier(id).subscribe((produitsPanier : ProduitPanier[]) => {
      this.userPanier = produitsPanier;
    });
  }

  onAjout(form: NgForm, nomP: string, typeP: string, prixP: number) {
    let id = JSON.parse(atob(this.userService.getToken().split('.')[1]))._id;
    let produitPanier = {
      userId: id,
      nomProduit: nomP,
    ...
    this.panierService.ajoutProduitPanier(produitPanier).subscribe(
      ...
      (err) => {
        this.flashMessage.show(err.error.join('<br/>'), {
  ...
  onSupprimer(id: String) {
    this.panierService.supprimeProduitPanier(id).subscribe(
      (res) => {
        if (res.success) {
          this.flashMessage.show(
            'Vous avez supprimé un produit de votre panier !',
            { cssClass: 'alert-success', timeout: 2000 }
          );
          location.reload();
        }
  ...
  onValidation() {
    ...
    this.panierService.validerPanier(id).subscribe(
      (res) => {
        this.flashMessage.show('Votre panier a été validé!', {
